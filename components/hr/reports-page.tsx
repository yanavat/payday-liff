...
export function ReportsPageContent() {
  ...
  const chartData = view === 'monthly' ? dailyDisbursements.slice(0, 30) : weeklyDisbursements
  const maxAmount = Math.max(...chartData.map((item) => item.amount), 1)
  const approvalRate = monthlySummary.totalApproved / Math.max(monthlySummary.totalRequests, 1) * 100
  ...
  return (
    <div className="max-w-[1152px] space-y-4">
      <header className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold leading-[28.6px] text-text-primary">รายงาน EWA</h1>
          <p className="mt-1 text-[13px] text-text-secondary">หน้าหลัก / รายงาน</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => exportReport('CSV')} className="inline-flex h-9 items-center gap-2 rounded-md border border-border bg-white px-3 text-sm font-medium hover:bg-bg-secondary">
            <Download className="h-4 w-4" /> Export CSV
          </button>
          <button onClick={() => exportReport('PDF')} className="inline-flex h-9 items-center gap-2 rounded-md border border-border bg-white px-3 text-sm font-medium hover:bg-bg-secondary">
      ...
      <div className="flex justify-end">
        <div className="flex rounded-md bg-bg-secondary p-1">
          <ToggleButton active={view === 'monthly'} onClick={() => setView('monthly')}>รายเดือน</ToggleButton>
          <ToggleButton active={view === 'weekly'} onClick={() => setView('weekly')}>รายสัปดาห์</ToggleButton>
        </div>
      </div>

      <div className="grid grid-cols-5 gap-3">
        <MetricCard label="ยอดเบิกจ่ายทั้งหมด" value={formatTHBCompact(monthlySummary.totalDisbursed)} />
        <MetricCard label="จำนวนคำขอ" value={`${monthlySummary.totalRequests}`} sub="รายการ" />
        <MetricCard label="เฉลี่ยต่อคำขอ" value={formatTHBCompact(monthlySummary.avgAmount)} />
      ...
      <section className="rounded-lg border border-border bg-white p-5 shadow-card">
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="text-caption text-text-muted">{view === 'monthly' ? 'รายวัน 30 แท่ง' : 'รายสัปดาห์ 52 แท่ง'}</span>
        </div>
        <div className="flex h-72 items-end gap-2 overflow-x-auto border-b border-border px-2 pb-6">
          {chartData.map((item, index) => {
            const height = Math.max((item.amount / maxAmount) * 220, item.amount > 0 ? 12 : 2)
            const highlighted = view === 'monthly' ? index === 13 : index === 18
            return (
              <div key={'date' in item ? item.date : item.week} className="flex min-w-[24px] flex-col items-center justify-end gap-2">
                <span className="font-number text-[10px] font-medium text-text-muted">{item.count}</span>
                <div
                  className={cn('w-6 rounded-t bg-primary-light', highlighted && 'bg-primary-dark')}
                  style={{ height }}
                  title={`${formatTHB(item.amount)} · ${item.count} รายการ`}
                />
                <span className="text-[10px] text-text-muted">{'date' in item ? String(index + 1) : item.week.replace('W', '')}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="grid grid-cols-[1fr_420px] gap-3">
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              ...
              <tr className="h-[52px] bg-bg-secondary font-semibold">
                ...
                <td className="px-4 text-right font-number text-sm">{formatTHB(Math.round(totals.totalAmount / totals.totalRequests))}</td>
              </tr>
            </tbody>
          </table>
        </section>
        ...
        <section className="overflow-hidden rounded-lg border border-border bg-white shadow-card">
          <div className="border-b border-border px-5 py-3.5">
            <h2 className="text-section-title text-text-primary">สถานะการโอนเงิน</h2>
          </div>
          <div className="divide-y divide-border-light">
            {reconciliationItems.map((item) => {
              ...
              return (
                <div key={item.referenceNumber} className={cn('p-4', item.status === 'failed' && !retried && 'bg-red-50/60')}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-number text-sm font-semibold">{item.referenceNumber}</p>
                      <p className="mt-1 text-caption text-text-muted">{item.employeeId} · {formatTHB(item.amount)}</p>
                    </div>
                  ...
                  {item.status === 'failed' && !retried && (
                    <button
                      onClick={() => {
                        setFailedRetried((current) => current.concat(item.referenceNumber))
                        toast({ variant: 'info', message: 'กำลังลองโอนเงินอีกครั้ง' })
                      }}
...
function ToggleButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={cn('h-8 rounded-sm px-4 text-sm font-medium', active ? 'bg-white text-text-primary shadow-card' : 'text-text-muted')}>
      {children}
    </button>
  )
}
