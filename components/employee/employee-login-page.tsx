  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col justify-center overflow-hidden bg-primary-bg px-5 py-8">
      ...
      <div className="absolute right-4 top-4 z-10 flex rounded-full border border-primary/20 bg-white p-1 shadow-card">
        {['TH', 'EN', 'MM'].map((item) => (
          <button
            ...
            className={cn(
              'h-8 min-w-10 rounded-full px-2 text-[12px] font-semibold',
      ...
      <div className="relative z-10 mb-8 flex flex-col items-center">
        ...
        <h1 className="text-2xl font-bold leading-tight text-primary">PayDay+</h1>
        <p className="mt-1 text-[16px] text-text-muted">เบิกเงินเดือนล่วงหน้า ง่าย รวดเร็ว</p>
      </div>
      ...
      <section className="relative z-10 rounded-xl bg-white p-6 shadow-modal">
        ...
        <label className="mb-2 block text-[16px] font-semibold text-text-primary" htmlFor="employee-id">
          รหัสพนักงาน
        </label>
        <input
          id="employee-id"
        ...
        <div className="mb-3 text-center text-[16px] font-semibold text-text-primary">PIN 4 หลัก</div>
        <PINPad
          value={pin}
          onChange={setPin}
        ...
        <button
          ...
          onClick={() => submitLogin()}
          className="mt-6 flex h-[52px] w-full items-center justify-center rounded-md bg-primary text-[16px] font-semibold text-white shadow-card transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-50"
        >
          {isLoading ? 'กำลังเข้าสู่ระบบ...' : 'เข้าสู่ระบบ'}
        </button>

        <button type="button" className="mt-4 w-full text-center text-[16px] font-medium text-primary">
          ลืม PIN?
        </button>
        <p className="mt-1 text-center text-[13px] text-text-muted">ติดต่อ HR ชั้น 1 โทร. 02-xxx-xxxx</p>

        <button
          type="button"
          disabled={isLocked}
          className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-md border border-primary bg-white text-[16px] font-semibold text-primary disabled:opacity-50"
        >
          <QrCode className="h-5 w-5" aria-hidden />
          สแกน QR บัตรพนักงาน
        </button>
        {!isLocked && attempts > 0 && (
          <p className="mt-3 text-center text-[13px] text-text-muted">เหลือโอกาสเข้าสู่ระบบ {remainingAttempts} ครั้ง</p>
        )}
      </section>
    </div>
  )
