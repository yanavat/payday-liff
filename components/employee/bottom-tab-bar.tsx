...
export function BottomTabBar() {
  ...
  const pendingCount = getRequestsByEmployee(currentEmployee.id).filter((request) => request.status === 'pending').length
        ...
        return (
          ...
          >
            <span className="relative">
              ...
              {showBadge && (
                <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-400 px-1 text-[10px] font-bold text-amber-950">
                  {pendingCount}
                </span>
              )}
            </span>
